...
class LogisticRegression(object):
    ...
    def loss(self, X_batch, Y_batch):
        ...
        y_predicted = sigmoid(np.matmul(X_batch, self.weights))
        loss = 0
        for i in range(len(y_predicted)):
            temp1 = Y_batch[i]*np.log(y_predicted[i])
            temp2 = np.subtract(1, Y_batch[i])
            temp3 = np.log(np.subtract(1, y_predicted[i]))
            loss = loss - temp1 - temp2*temp3
        loss = loss/len(Y_batch)
        grad = (1/len(Y_batch))*np.matmul(X_batch.T, np.subtract(y_predicted, Y_batch))
    ...
    def train(self,
        ...
        if self.add_bias:
            self.weights = np.random.normal(size = X_train.shape[1]+1) # Fill a correct value here
        else:
            self.weights = np.random.normal(size = X_train.shape[1])  # Fill a correct value here
        ...
        # while taking in consideration the bias (so two cases according to the boolean value of add_bias).
        if self.add_bias:
            X_train_adj = np.insert(X_train,X_train.shape[1],1,axis=1)
        else:
            X_train_adj = X_train
        ...
        for i in range(num_iters):
            ...
            loss = loss + (reg_const/(2*(X_train_adj.shape[1])))*(np.linalg.norm(self.weights)**2)
